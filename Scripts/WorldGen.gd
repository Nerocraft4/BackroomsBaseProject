...
onready var motiontracker = $MotionLowRes
var references = {'a': preload("res://prefabs/MotionLowRes.tscn")}
...
func _input(event):
	if event is InputEventKey and event.pressed:
		if event.scancode == KEY_F:
			var stats = get_save_stats()
			print(stats)
			load_save_stats(stats)
...
func load_save_stats(stats):
	if stats.items.a:
		var instance = references.a.instance()
		add_child(instance)
